...
def test_two_overviews_normal(testdata, tmp_path, pdftester):
    ...
    assert pdftester.pagesize("mock_1.pdf") == [4744.448, 3354.859]
    assert pdftester.pagesize("mock_2.pdf") == [4151.392, 2516.144]

    assert pdftester.pattern_pages_order("mock_1.pdf") == ["25A", "8A"]
    assert pdftester.pattern_pages_order("mock_2.pdf") == ["15B", "7B"]
