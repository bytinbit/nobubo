...
def assemble_collage(input_pdf: pathlib.Path,  # adapted
    ...
    page_width = input_properties.input_papersize.width
    page_height = input_properties.input_papersize.height
...
def _calculate_lowerleft_point(lowerleft_factor: utils.Factor,
    ...
    return utils.Point(x=lowerleft_factor.x * n_up_factor.x * input_properties.input_papersize.width,
                       y=lowerleft_factor.y * n_up_factor.y * input_properties.input_papersize.height)
...
def _calculate_upperright_point(upperright_factor: utils.Factor,
    ...
    if rowsleft < 0:  # end of pattern reached  (full amount of rows reached)
        upperright.y = layout.rows * input_properties.input_papersize.height
    else:
        upperright.y = upperright_factor.y * n_up_factor.y * input_properties.input_papersize.height
    ...
    if colsleft > 0:  # still assembling the same horizontal line
        upperright.x = upperright_factor.x * n_up_factor.x * input_properties.input_papersize.width

    else:  # end of line reached, need to go 1 row up
        if colsleft == 0:  # cols % n_up_factor == 0
            upperright.x = upperright_factor.x * n_up_factor.x * input_properties.input_papersize.width
        if colsleft < 0:  # remainder pages left for COLS
            upperright.x = layout.columns * input_properties.input_papersize.width
    return upperright
